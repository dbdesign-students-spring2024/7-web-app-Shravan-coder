...
try:
    cxn = pymongo.MongoClient(os.getenv('class-mongodb.cims.nyu.edu'), 27017, 
                                username=os.getenv('sp7055'),
                                password=os.getenv('Cx4fiZFN'),
                                authSource=os.getenv('sp7055'))
...
def create_post():
    ...
    show = request.form["fshow"]
    rating = request.form["frating"]
    review = request.form["freview"]


    # create a new document with the data the user entered
    doc = {"name": name, "show": show, "rating": rating, "review": review, "created_at": datetime.datetime.utcnow()}
...
@app.route("/details/<mongoid>")
def details(mongoid):
    """
    Route for displaying details about a document.
    
    Parameters:
    mongoid (str): The MongoDB ObjectId of the document.
    """
    # Add your logic here to fetch and display details about the document with the provided mongoid
    # Fetch the review from the database based on the provided mongoid
    docs = db.exampleapp.find_one({"_id": ObjectId(mongoid)})
    print("Review:", docs)  # Debug print statement

    # Pass the review data to the template for rendering
    return render_template("details.html", docs=docs)
